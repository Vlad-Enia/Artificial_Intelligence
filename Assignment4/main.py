...
class NeuralNetwork:
    ...
    def train(self, train_set, test_set, iterations_number, batch_size, learning_rate,momentum_coefficient,l2_constant):
...
def read_training_data(path):
    ...
    for line_ in data:
        line_ = line_.split(' ')
        x_aux.append([int(line_[0]), int(line_[1])])
        y_aux.append(int(line_[2]))
    return np.array(x_aux), np.array(y_aux)

train_set=[[],[]]
train_set[0],train_set[1]=read_training_data('XOR.txt')
print(train_set)
IDENTITY_MATRIX = np.array([1 if i == j else 0 for i in range(2) for j in range(2)]).reshape(2,2)  # Matrix of 0 except when i=j the value is 0
ITERATIONS = 50
LEARNING_RATE = 0.3
MINI_BATCH_SIZE = 2
MOMENTUM_COEFFICIENT=0.000005
...
mnist_classifier = NeuralNetwork(len(train_set[0][0]), 20, 2)
mnist_classifier.train(train_set, train_set, ITERATIONS, MINI_BATCH_SIZE, LEARNING_RATE,MOMENTUM_COEFFICIENT,L2_CONSTANT)
